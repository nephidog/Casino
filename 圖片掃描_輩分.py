...
def Check_Adjacent(Point,Bar,Blurry=1): #判斷傳進來的點是否相鄰
    # Blurry = (Blurry**2 + Blurry** 2 )*0.5
    x1,y1,x2,y2 = Bar
    x1 -= Blurry
    y1 -= Blurry
...
while len(box)>0:
    ...
    while True:
        Count = 0
        # plan_len = plan(Bar)
        # x1,y1,x2,y2 = Point_Min_Max(PointList)
        for i in box:
            # if plan_len == 0 or plan((Bar[2],Bar[3],i[0],i[1])) <= plan_len:
                if Check_Adjacent(i,Bar,5):
